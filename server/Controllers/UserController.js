...
const Register = async(req,res)=>{
    try {
        ...
        if(!errors.isEmpty()){
            return res.status(401).json({errors: errors.mapped()})  // err lel client ouw btest7agha kalmet errors zay ma hia fel slice ba3tali
        }
        ...
        const {name,age,email,password} = req.body
         ...
         if(found){
             return res.status(400).json({message:'User already exists'})  // err lel client ouw btest7agha kalmet message zay ma hia fel slice ba3tali
         }
